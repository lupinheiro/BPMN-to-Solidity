...
// Get de todos os Process Registry de todos os contratos.
...
// Get de todos os modelos criados.
...
// Configuracao dos Bindings (Users -> Tarefas).

//Post da policy que mapeia os users as tarefas.
...
//Associa uma funcao a um processo através da Binding Policy.
...
let registerFactory = (currentIndex, sortedElements, outputContracts, modelInfo, response) => {
    ...
    FactoryContract.new({ from: web3.eth.accounts[0], data: "0x" + outputContracts[entryFactoryName].bytecode, gas: 4700000 }, (errF, contractF) => {
        if (errF) {
...
let createWorklistInstances = (currentIndex, sortedElements, outputContracts, modelInfo, response) => {
    ...
    if (outputContracts[entryWorklistName]) {
        ...
        WorklistContract.new({ from: web3.eth.accounts[0], data: "0x" + outputContracts[entryWorklistName].bytecode, gas: 4700000 }, (errW, contractW) => {
            if (errW) {
